...
import { resolve, dirname, normalize, basename, extname, relative, join, parse as parse$2 } from 'path';
import { statSync, readdirSync, readFileSync, writeFile, mkdirSync, writeFileSync } from 'fs';
...
class Y18N {
    ...
    _resolveLocaleFile(directory, locale) {
        var file = shim$1.resolve(directory, './', locale + '.json');
        if (this.fallbackToLanguage && !this._fileExistsSync(file) && ~locale.lastIndexOf('_')) {
            // attempt fallback to language only
            var languageFile = shim$1.resolve(directory, './', locale.split('_')[0] + '.json');
...
function applyExtends(config, cwd, mergeExtends, _shim) {
    ...
    if (Object.prototype.hasOwnProperty.call(config, 'extends')) {
        ...
        defaultConfig = isPath
            ? JSON.parse(shim$2.readFileSync(pathToDefault, 'utf8'))
            : require(config.extends);
        delete config.extends;
        defaultConfig = applyExtends(defaultConfig, shim$2.path.dirname(pathToDefault), mergeExtends, shim$2);
    }
...
function getPathToDefaultConfig(cwd, pathToExtend) {
    return shim$2.path.resolve(cwd, pathToExtend);
}
...
function Yargs(processArgs = [], cwd = shim$3.process.cwd(), parentRequire) {
    const self = {};
    let command$1;
    ...
    let default$0;
    if (/\b(node|iojs|electron)(\.exe)?$/.test(shim$3.process.argv()[0])) {
        default$0 = shim$3.process.argv().slice(1, 2);
    }
    else {
        default$0 = shim$3.process.argv().slice(0, 1);
    }
    self.$0 = default$0
        .map(x => {
        ...
        .join(' ')
        .trim();
    if (shim$3.getEnv('_') && shim$3.getProcessArgvBin() === shim$3.getEnv('_')) {
        self.$0 = shim$3
            .getEnv('_')
            .replace(`${shim$3.path.dirname(shim$3.process.execPath())}/`, '');
    }
    ...
    self.resetOptions = self.reset = function resetOptions(aliases = {}) {
        ...
        options = tmpOptions;
        usage$1 = usage$1 ? usage$1.reset(localLookup) : usage(self, y18n, shim$3);
        validation$1 = validation$1
            ? validation$1.reset(localLookup)
            : validation(self, usage$1, y18n, shim$3);
        command$1 = command$1
            ? command$1.reset()
            : command(self, usage$1, validation$1, globalMiddleware, shim$3);
        if (!completion$1)
            completion$1 = completion(self, usage$1, command$1, shim$3);
    ...
    function unfreeze() {
        const frozen = frozens.pop();
        assertNotStrictEqual(frozen, undefined, shim$3);
        let configObjects;
        ({
            options,
            configObjects,
    ...
    self.config = function config(key = 'config', msg, parseFn) {
        ...
        if (typeof key === 'object' && !Array.isArray(key)) {
            key = applyExtends(key, cwd, self.getParserConfiguration()['deep-merge-config'] || false, shim$3);
    ...
    self.commandDir = function (dir, opts) {
        ...
        const req = parentRequire || shim$3.require;
        command$1.addDirectory(dir, self.getContext(), req, shim$3.getCallerFile(), opts);
        return self;
    };
    self.demand = self.required = self.require = function demand(keys, max, msg) {
        if (Array.isArray(max)) {
            max.forEach(key => {
                assertNotStrictEqual(msg, true, shim$3);
    ...
    self.usage = function (msg, description, builder, handler) {
        ...
        if (description !== undefined) {
            assertNotStrictEqual(msg, null, shim$3);
            if ((msg || '').match(/^\$0( |$)/)) {
    ...
    self.pkgConf = function pkgConf(key, rootPath) {
        ...
        if (obj[key] && typeof obj[key] === 'object') {
            conf = applyExtends(obj[key], rootPath || cwd, self.getParserConfiguration()['deep-merge-config'] || false, shim$3);
    ...
    function pkgUp(rootPath) {
        ...
        let obj = {};
        try {
            let startDir = rootPath || shim$3.mainFilename;
            if (!rootPath && shim$3.path.extname(startDir)) {
                startDir = shim$3.path.dirname(startDir);
            }
            const pkgJsonPath = shim$3.findUp(startDir, (dir, names) => {
            ...
            assertNotStrictEqual(pkgJsonPath, undefined, shim$3);
            obj = JSON.parse(shim$3.readFileSync(pkgJsonPath, 'utf8'));
        }
        catch (_noop) { }
        pkgs[npath] = obj || {};
        return pkgs[npath];
    ...
    self.terminalWidth = () => {
        argsert([], 0);
        return shim$3.process.stdColumns;
    };
    ...
    self._parseArgs = function parseArgs(args, shortCircuit, _calledFromCommand, commandIndex) {
        ...
        const parsed = shim$3.Parser.detailed(args, Object.assign({}, options, {
    ...
    self._parsePositionalNumbers = function (argv) {
        ...
        for (let i = 0, arg; (arg = args[i]) !== undefined; i++) {
            if (shim$3.Parser.looksLikeNumber(arg) &&
    ...
    function guessLocale() {
        if (!detectLocale)
            return;
        const locale = shim$3.getEnv('LC_ALL') ||
            shim$3.getEnv('LC_MESSAGES') ||
            shim$3.getEnv('LANG') ||
            shim$3.getEnv('LANGUAGE') ||
            'en_US';
...
const rebase = (base, dir) => shim$3.path.relative(base, dir);
function isYargsInstance(y) {
    return !!y && typeof y._parseArgs === 'function';
}

const Yargs$1 = YargsWithShim(esmPlatformShim);
...
const config_schema = object({
    ...
    applications: array(object({
        ...
        image_url: string$1(),
        port: number(),
        env: defaulted(record(string$1(), string$1()), {})
    }))
});
const check_config = (u) => create(u, config_schema);
async function load_insidethesandbox_config(relative_config_path = './insidethesandbox.config.mjs') {
    const config_path = join(process.cwd(), relative_config_path);
    const raw_config = (await import(config_path)).default;
    const raw_config2 = lodash.merge(raw_config, {
        applications: raw_config.applications.map((a) => {
...
const github_action_builder = (argv) => argv
    .option('config', {
    alias: 'c',
...
async function github_action_tempalte(args) {
    const workflow_folder = join(process.cwd(), args.output);
    mkdirSync(workflow_folder, { recursive: true });
...
function deployment({ name, namespace, replicas = 1, image_url = `registry.digitalocean.com/insidethesandbox/${namespace}-${name}:latest`, container_port = 80, image_pull_secret = IMAGE_PULL_SECRET, env }) {
    return {
        apiVersion: 'apps/v1',
        kind: 'Deployment',
        metadata: {
                    ...
                    containers: [
                        {
                            ...
                            env: lodash.map(env, (val, key) => ({ name: key, value: `${val}` })),
                            resources: {
...
function service({ name, namespace, port = 80 }) {
    return {
        apiVersion: 'v1',
        kind: 'Service',
        metadata: {
...
function certificate({ namespace, tls_crt, tls_key }) {
    return {
        apiVersion: 'v1',
        kind: 'Secret',
        metadata: {
...
function ingress({ applications, namespace }) {
    return {
...
async function kube_generate_config(args) {
    const config = await load_insidethesandbox_config(args.config);
    const k8s_config = lodash.flattenDeep([
        namespace({ namespace: config.kubernetes.namespace }),
        certificate({ namespace: config.kubernetes.namespace, tls_crt: config.cloudflare.crt, tls_key: config.cloudflare.key }),
        ingress({ namespace: config.kubernetes.namespace, applications: config.applications }),
        config.applications.map(app => {
            var _a;
            return [
                service({ namespace: config.kubernetes.namespace, name: app.name, port: app.port }),
                deployment({ namespace: config.kubernetes.namespace, name: app.name, env: (_a = app.env) !== null && _a !== void 0 ? _a : {} }),
            ];
        })
    ]);
    const out = args.format === 'json'
        ? JSON.stringify(k8s_config, null, 2)
        : lodash.join(k8s_config.map(c => yaml.stringify(c)), '---\n');
    if (args.output === 'stdout') {
        console.log(out);
    }
    else {
        const out_path = join(process.cwd(), args.output);
        mkdirSync(parse$2(out_path).dir, { recursive: true });
        writeFileSync(join(process.cwd(), args.output), out);
    }
}

Yargs$1(process.argv.slice(2), process.cwd())
